...
test('supports the owner and borrower lending lifecycle', async ({ browser }, testInfo) => {
  ...
  try {
    ...
    const detailLoan = ownerPage.getByRole('region', { name: 'Loan' })
    await expect(detailLoan.getByText('Lent out', { exact: true })).toBeVisible()
    await detailLoan.getByRole('button', { name: 'More loan actions' }).click()
    await ownerPage.getByRole('menuitem', { name: 'Edit loan note' }).click()
    const detailLoanNoteModal = ownerPage.getByRole('dialog', { name: 'Loan note' })
    await detailLoanNoteModal.getByPlaceholder('Add a private note about this loan...').fill('Updated private owner note')
    await detailLoanNoteModal.getByPlaceholder('Add a private note about this loan...').press('Meta+Enter')
    await expect(detailLoanNoteModal).not.toBeVisible()
    await expect(detailLoan.getByText('Updated private owner note')).toBeVisible()
