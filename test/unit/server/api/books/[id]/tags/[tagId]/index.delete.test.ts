...
const route = routePath('books/[id]/tags/[tagId]/index.delete')

describe('server/api/books/[id]/tags/[tagId]/index.delete', () => {
  beforeEach(setupApiRouteTest)
  afterEach(cleanupApiRouteTest)

  itRequiresAuth(route, { params: { id: 'ub-1', tagId: 'tag-1' } })

  it('deletes a tag from a book', async () => {
    mockLoggedInUser()
    serviceMocks.deleteTag.mockReturnValueOnce(Effect.succeed(undefined))
    const handler = await importRoute(route)

    await expect(handler(makeEvent({ params: { id: 'ub-1', tagId: 'tag-1' } }))).resolves.toEqual({ success: true })
    expect(serviceMocks.deleteTag).toHaveBeenCalledWith('ub-1', 'user-1', 'tag-1')
  })
