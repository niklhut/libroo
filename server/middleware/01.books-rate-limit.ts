...
import { getBooksEnrichmentRateLimitConfig, getBooksRateLimitConfig, getBulkLookupRateLimitConfig } from '../utils/books-config'
import { DatabaseRateLimiter, redactKey } from '../utils/database-rate-limiter'
import { runEffect } from '../utils/effect'

const BOOKS_RATE_LIMIT_PATHS = new Set(['/api/books/lookup', '/api/books', '/api/books/bulk-lookup', '/api/books/enrichment/run', '/api/books/enrichment/run-batch', '/api/books/enrichment/batch'])
...
export default defineEventHandler(async (event) => {
  ...
  const config = event.path === '/api/books/bulk-lookup'
    ...
    : event.path === '/api/books/enrichment/run' || event.path === '/api/books/enrichment/run-batch' || event.path === '/api/books/enrichment/batch'
